...
const SEED_ACCOUNTS = [
    { username: 'Diaxgalaksija', apikey: 'bf29b66336128151', nickname: 'Diaxgalaksija' },
];
...
app.patch('/api/accounts/:id', requireAuth, safe(async (req, res) => {
    ...
    // Lista auto-poruka - svaka poruka je { text, delaySeconds }, gde je
    // delaySeconds vreme (u sekundama) čekanja nakon prethodne poruke ovog
    // naloga, pre slanja baš te poruke.
    if (req.body?.messages !== undefined) {
        ...
        if (bot) {
            try {
                await bot.setMessages(account.messages);
            } catch (error) {
    ...
    // Uključivanje/isključivanje !ai komande za ovaj nalog (kvadratić u
    // panelu) - primenjuje se odmah, bez restarta bota.
    if (req.body?.aiEnabled !== undefined) {
        const aiEnabled = req.body.aiEnabled !== false;
        account.aiEnabled = aiEnabled;

        if (bot) {
            try {
                await bot.setAiEnabled(aiEnabled);
            } catch (error) {
                console.error(`Greška pri promeni aiEnabled: ${error.message}`);
            }
        }
    }
